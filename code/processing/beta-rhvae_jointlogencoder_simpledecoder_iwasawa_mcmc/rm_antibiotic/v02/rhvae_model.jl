...
# Define numebr of environments to remove from the model
n_rm_env = 1
...
encoder_chain = Flux.Chain(
    # First layer
    Flux.Dense(n_env - n_rm_env => n_neuron, Flux.identity),
    # Second layer
...
decoder = AutoEncoderToolkit.SimpleGaussianDecoder(
    Flux.Chain(
        ...
        Flux.Dense(n_neuron => n_neuron, Flux.leakyrelu),
        # Output layer
        Flux.Dense(n_neuron => n_env - n_rm_env, Flux.identity)
    )
)

# Initialize second decoder for missing environment
decoder_missing = AutoEncoderToolkit.SimpleGaussianDecoder(
    Flux.Chain(
        # First layer
        Flux.Dense(n_latent => n_neuron, Flux.identity),
        # Second Layer
        ...
        Flux.Dense(n_neuron => n_neuron, Flux.leakyrelu),
        # Output layer
        Flux.Dense(n_neuron => n_rm_env, Flux.identity)
    )
)
...
# Loop over number of environments to remove
for n_rm_env in 1:n_env
    println("- Saving model for $(drug_list[n_rm_env]) removal...")
    # Define drug to be removed
    drug_rm = drug_list[n_rm_env]

    # List all but the index of the drug to be removed
    env_list = sort(setdiff(1:n_env, n_rm_env))

    # Select centroids via k-means
    centroids_data = AutoEncoderToolkit.utils.centroids_kmedoids(
        logic50_mean_std[env_list, :], n_centroids
    )
    ...
    # Save model object
    JLD2.save(
        "$(out_dir)/model_$(drug_rm)rm.jld2",
        Dict("model" => rhvae, "model_state" => Flux.state(rhvae))
    )
end

println("Save second decoder for missing environment...")
# Save second decoder
JLD2.save(
    "$(out_dir)/decoder_missing.jld2",
    Dict("model" => decoder_missing, "model_state" => Flux.state(decoder_missing))
)
