...
for (i, drug) in enumerate(drug_list)
    ...
    # Load model
    rhvae = JLD2.load("$(model_dir)/model_$(drug)rm.jld2")["model"]
    # Load parameters
    model_state = JLD2.load("$(model_dir)/model_$(drug)rm.jld2")["model_state"]
    ...
    model_states = sort(Glob.glob("$(out_dir)/beta-rhvae_$(drug)rm_epoch*.jld2"[2:end], "/"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/beta-rhvae_$(drug)rm_epoch$(lpad(epoch, 5, "0")).jld2",
