...
out_dir = "$(git_root())/output$(prefix)/vae_model_state"
...
for (i, drug) in enumerate(drug_list)
    ...
    train_data = data[env_list, train_idx, :] |> Flux.gpu
    val_data = data[env_list, val_idx, :] |> Flux.gpu
    ...
    # Load model
    vae = JLD2.load("$(model_dir)/vae_model_base.jld2")["model"]
    # Load parameters
    model_state = JLD2.load("$(model_dir)/vae_model_base.jld2")["model_state"]
    ...
    model_states = sort(Glob.glob("$(out_dir)/beta-vae_$(drug)rm_epoch*.jld2"[2:end], "/"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        for (i, idx_tuple) in enumerate(idx_batches)
            ...
            loss_epoch = AET.VAEs.train!(
                vae, train_data[:, idx_batch, idx_mcmc], opt_vae;
        ...
        if epoch == n_epoch
            JLD2.jldsave(
                "$(out_dir)/beta-vae_$(drug)rm_epoch$(lpad(epoch, 5, "0")).jld2",
