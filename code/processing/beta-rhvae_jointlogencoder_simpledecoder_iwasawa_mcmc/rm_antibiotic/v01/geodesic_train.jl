...
model_dir = "$(git_root())/output$(out_prefix)/rhvae_model_state"
...
drug_list = JLD2.load("$(data_dir)/logic50_preprocess.jld2")["drugs"]
...
# Add strain and evo columns to dataframe by splitting the env column
df_logic50.strain = [split(env, "_")[1] for env in df_logic50.env]
df_logic50.evo = [split(env, "_")[end] for env in df_logic50.env]

# Loop through drugs
for drug in drugs
    println("Loading RHVAE model for $(drug)...")

    # Load RHVAE model
    rhvae = JLD2.load("$(out_dir)/model_$(drug)rm.jld2")["model"]
    # List parameters for epochs
    param_files = sort(Glob.glob("$(model_dir)/*$(drug)rm*.jld2"[2:end], "/"))
    # Load last epoch
    ...
    println("Map data without $(drug) to latent space...")

    # Extract data without drug
    df_logic50_full = df_logic50[(df_logic50.drug.≠drug).&(df_logic50.evo.==drug), :]
    # Group dataframe by :day, :strain_num, and :env
    df_group = DF.groupby(df_logic50_full, [:day, :strain_num, :env])
    ...
    # Initialize empty dataframe to store data from IDs to be selected for
    # geodesic analysis
    ...
    Threads.@threads for i in 1:length(df_group)
        ...
        println("Processing lineage: $(i)/$(length(df_group)) | $(env) | $(strain_num) | $(parent) | $(drug) removed")
        # Find minimum day
        ...
        for epoch in 1:n_epoch
            if epoch % 2_500 == 0
                println("       - Epoch: $(epoch) | $(i)/$(length(df_group))")
            end
            # Train model and save loss
            nng_loss[epoch] = NG.train!(
                nng, rhvae, t_array, opt_nng; loss_return=true
            )
        end # for
        # Save network
        JLD2.jldsave(
