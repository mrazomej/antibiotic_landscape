...
state_dir = "$(git_root())/output$(prefix)/vae_model_state"
# Define output directory
out_dir = "$(git_root())/output$(prefix)/vae_crossvalidation_state"
...
for (i, drug) in enumerate(drug_list)
    println("Training VAE for $(drug) removal...")
    ...
    # Load model
    vae_full = JLD2.load("$(model_dir)/vae_model_base.jld2")["model"]
    # List parameters
    param_files = sort(Glob.glob("$(state_dir)/beta-vae_$(drug)rm_epoch*.jld2"[2:end], "/"))
    ...
    # Build second rhvae
    vae = deepcopy(vae_full.encoder) * decoder_missing
    ...
    model_states = sort(Glob.glob("$(out_dir)/beta-vae_$(drug)rm_crossval_$(split_frac)split_epoch*.jld2"[2:end], "/"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        # Save checkpoint if it is the last epoch, else, save loss and mse
        if epoch == n_epoch
            JLD2.jldsave(
                "$(out_dir)/beta-vae_$(drug)rm_crossval_$(split_frac)split_epoch$(lpad(epoch, 5, "0")).jld2",
