...
# Group dataframe by :strain_num
df_group = DF.groupby(df_latent, :strain_num)
...
for (i, data) in enumerate(df_group)
    ...
    scatter!(
        ...
        data.latent1,
        data.latent2,
        label="$(first(data.strain_num))",
        markersize=7,
        color=ColorSchemes.glasbey_hv_n256[i],
    )
end # for

save("$(fig_dir)/rhvae_latent_space_strain_num.pdf", fig)
save("$(fig_dir)/rhvae_latent_space_strain_num.png", fig)
