...
# Extract unique evolution conditions
evolution = unique(df_meta.evolution)

# Loop through each evolution condition
for evo in evolution
    println("Processing evolution condition: $evo")
    ...
    println("Select training and validation data...")
    # Extract indexes to keep
    idx_keep = df_meta.evolution .≠ evo
    idx_rm = .!idx_keep
    # Extract data to keep
    train_data = data[:, idx_keep, :] |> Flux.gpu
    # Extract data to remove
    ...
    model_states = sort(
        Glob.glob("$(out_dir)/beta-rhvae_$(evo)_epoch*.jld2"[2:end], "/")
    )
    ...
    for epoch in epoch_init:n_epoch
        ...
        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/beta-rhvae_$(evo)_epoch$(lpad(epoch, 5, "0")).jld2",
