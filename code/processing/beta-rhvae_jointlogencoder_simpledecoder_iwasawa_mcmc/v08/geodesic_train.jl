...
# Convert grouped data to a vector for threading
group_vector = collect(df_group)

# Initialize a progress counter
progress_counter = Threads.Atomic{Int}(0)

# Use threading for the loop
Threads.@threads for i in 1:length(group_vector)
    data = group_vector[i]
    # Sort data by day
    DF.sort!(data, :day)
    # Define metadata
    ...
    println("Processing lineage: $(current_progress)/$(length(group_vector)) | $(env) | $(strain_num) | $(parent)")
