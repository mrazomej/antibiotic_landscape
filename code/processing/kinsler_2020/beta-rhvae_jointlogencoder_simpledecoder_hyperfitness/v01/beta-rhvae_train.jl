...
df_kinsler_std = DF.unstack(df_kinsler, :env, :id, :fitness_std_standard)

# Extract fitness matrix
data_mean = Float32.(Matrix(df_kinsler_mean[:, DF.Not(:env)]))
data_std = Float32.(Matrix(df_kinsler_std[:, DF.Not(:env)]))
...
train_data_std = data_std[:, train_idx] |> Flux.gpu
val_data_mean = data_mean[:, val_idx] |> Flux.gpu
val_data_std = data_std[:, val_idx] |> Flux.gpu
...
for epoch in epoch_init:n_epoch
    ...
    for (i, idx_tuple) in enumerate(idx_batches)
        ...
        train_data = train_data_mean[:, idx_batch] .+
                     train_data_std[:, idx_batch] .*
                     CUDA.randn(
            Float32, size(train_data_mean, 1), length(idx_batch)
        )
        # Train RHVAE
    ...
    train_sample = train_data_mean .+ train_data_std .* CUDA.randn(
        Float32, size(train_data_mean)...
    )
    # Sample val data as mean + standard deviation * standard normal
    val_sample = val_data_mean .+ val_data_std .* CUDA.randn(
        Float32, size(val_data_mean)...
    )
