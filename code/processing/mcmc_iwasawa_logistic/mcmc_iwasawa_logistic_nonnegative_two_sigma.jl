...
out_dir = "$(git_root())/data/Iwasawa_2022/mcmc_nonnegative_two_sigma"
...
Turing.@model function logistic_model(
    ...
    σ²_left ~ Turing.truncated(Turing.Normal(params.σ²_left...), 0, Inf)
    σ²_right ~ Turing.truncated(Turing.Normal(params.σ²_right...), 0, Inf)

    # Compute model predictions
    μ = log.(logistic(logx, a, b, c, logic50))

    # Create variance vector based on IC50
    σ² = [x <= logic50 ? σ²_left : σ²_right for x in logx]

    # Define likelihood
    logy ~ Turing.MvNormal(μ, LinearAlgebra.Diagonal(σ²))
end
