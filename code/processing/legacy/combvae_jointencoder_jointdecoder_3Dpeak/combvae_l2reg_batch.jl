...
fname = "./output/$(n_epoch)_epoch/combvae_l2reg_$(n_latent)dimensions.bson"
...
# Initialize decoder
decoder = AutoEncode.VAEs.JointDecoder(
    n_input,
    n_latent,
    decoder_neurons,
    decoder_activation,
...
reg_fn(vae_outputs) = AutoEncode.regularization.l2_regularization(
    vae_outputs, Dict(:decoder_logσ => true)
)

# Define loss function with L2 regularization
loss(vae::AutoEncode.VAEs.VAE, data::AbstractArray,)
...
vae_mse[1] = StatsBase.mean(
    Flux.mse.(eachcol(data_std), eachcol(vae(data_std)[1]))
)
