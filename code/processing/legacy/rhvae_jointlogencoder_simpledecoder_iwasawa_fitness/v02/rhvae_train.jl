...
η_vals = [1E-5, 1E-4, 1E-3, 1E-2, 1E-1]
...
models = [Flux.deepcopy(rhvae_template) for _ in η_vals]
...
`n_epoch = $(n_epoch)`
## Training regimen
`rhvae mini-batch training witm multiple learning rate values for ADAM optimizer`
## Batch size
`n_batch = $(n_batch)`
## Data split fraction
...
Threads.@threads for i in eachindex(η_vals)
    # Define model
    rhvae = models[i]
    ...
    model_states = sort(Glob.glob("$(out_dir)/rhvae_$(η)learningrate_epoch*.jld2"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/rhvae__$(η)learningrate_epoch$(lpad(epoch, 5, "0")).jld2",
