...
# Define T values
T_vals = Float32.([0.1, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5])
...
`n_epoch = $(n_epoch)`
## Training regimen
`rhvae mini-batch training witm multiple T values`
## Lambda values
`T_vals = $(T_vals)`
## Batch size
`n_batch = $(n_batch)`
...
Threads.@threads for i in eachindex(T_vals)
    ...
    model_states = sort(Glob.glob("$(out_dir)/rhvae_$(T)temp_epoch*.jld2"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/rhvae_$(T)temp_epoch$(lpad(epoch, 5, "0")).jld2",
