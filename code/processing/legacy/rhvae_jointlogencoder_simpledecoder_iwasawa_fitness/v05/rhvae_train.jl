...
# Load models
rhvae_dict = JLD2.load("./output/models.jld2")

# Extract number of neurons from dictionary keys
n_neurons = sort(
    unique([parse(Int, split(x, "_")[1]) for x in keys(rhvae_dict)])
)
...
`latent_dim = $(size(rhvae_dict["128_model"].vae.encoder.µ.weight, 1))`
## Number of epochs
`n_epoch = $(n_epoch)`
## Training regimen
`rhvae mini-batch training witm multiple number of neurons `
## Number of neurons
`n_neurons = $(n_neurons)`
## Batch size
`n_batch = $(n_batch)`
...
Threads.@threads for i in eachindex(n_neurons)
    # Extract number of neurons
    n_neuron = n_neurons[i]
    # Load model
    rhvae = rhvae_dict["$(n_neuron)_model"]
    # Load model parameters
    model_state = rhvae_dict["$(n_neuron)_state"]
    ...
    model_states = sort(Glob.glob("$(out_dir)/rhvae_$(n_neuron)neurons_epoch*.jld2"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        println("\n ($(n_neuron)) Epoch: $(epoch) / $(n_epoch)\n - mse_train: $(mse_train)\n - mse_val: $(mse_val)\n")

        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/rhvae_$(n_neuron)neurons_epoch$(lpad(epoch, 5, "0")).jld2",
