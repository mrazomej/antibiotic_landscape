...
for epoch in epoch_init:n_epoch
    ...
    # Define β parameter
    β = AutoEncode.utils.cycle_anneal(epoch, n_epoch, n_cycle)
    ...
    AutoEncode.VAEs.train!(
        vae, data_batch, opt_vae; verbose=false, loss_kwargs=Dict(:β => β)
    )
    # Thaw decoder log(σ)
    ...
    if epoch % 100 == 0
        ...
        vae_outputs = vae(ic50_std)
        # Print MSE
        println("MSE: $(Flux.mse(vae_outputs.µ, ic50_std))")

        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/vae-cyc_epoch$(lpad(epoch, 5, "0")).jld2",
