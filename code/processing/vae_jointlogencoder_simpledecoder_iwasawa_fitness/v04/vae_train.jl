...
# Load model
model_dict = JLD2.load("./output/models.jld2")
...
readme = """
# `$(@__FILE__)`
# Multiple Variational Autoencoders with different latent dimensions
## Number of epochs
`n_epoch = $(n_epoch)`
## Training regimen
...
# Extract unique latent dimensions
dims = collect(keys(model_dict))

# Loop through latent dimensions
Threads.@threads for i in eachindex(dims)
    # Extract dimension
    dim = dims[i]
    # Extract model
    vae = model_dict[dim][:model]
    # Load model state
    Flux.loadmodel!(vae, model_dict[dim][:state])
    ...
    println("$(dim) | Initial MSE Train: $(mse_train)")
    println("$(dim) | Initial MSE Val: $(mse_val)\n")
    ...
    for epoch in 1:n_epoch
        ...
        for (i, x) in enumerate(train_loader)
            # Train vae
            AutoEncode.VAEs.train!(vae, x, opt_vae)
        end # for train_loader

        # Check if checkpoint should be saved
        if epoch % n_error == 0
            println("\n $(dim) | Epoch: $(epoch) / $(n_epoch)\n")
            ...
            # Save checkpoint
            JLD2.jldsave(
                "$(out_dir)/vae_$(dim)_epoch$(lpad(epoch, 5, "0")).jld2",
