...
fit_std = StatsBase.transform(dt, fit_mat)[1:n_env_train, :]
...
model_states = sort(Glob.glob("$(state_dir)/vae_epoch*.jld2"[2:end], "/"))
...
for epoch in epoch_init:n_epoch
    ...
    if epoch % 10 == 0
        ...
        JLD2.jldsave(
            "$(state_dir)/vae_epoch$(lpad(epoch, 5, "0")).jld2",
        ...
        # Only save loss metrics for other epochs
        JLD2.jldsave(
            "$(state_dir)/vae_epoch$(lpad(epoch, 5, "0")).jld2",
            loss_train=loss_train,
            loss_val=loss_val,
