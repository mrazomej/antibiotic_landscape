...
ϵ_vals = Float32.([1E-6, 1E-5, 1E-4, 1E-3]) # Leapfrog step size
...
`n_epoch = $(n_epoch)`
## Training regimen
`β-rhvae mini-batch training witm multiple leapfrog steps`
## Leapfrog steps
`ϵ_vals = $(ϵ_vals)`
## Batch size
`n_batch = $(n_batch)`
...
Threads.@threads for i in 1:length(ϵ_vals)
    # Define model
    ...
    model_states = sort(Glob.glob("$(out_dir)/beta-rhvae_$(ϵ)stepsize_*epoch.jld2"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        println("\n (ϵ = $(ϵ)) Epoch: $(epoch) / $(n_epoch)\n - mse_train: $(mse_train)\n - mse_val: $(mse_val)\n")

        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/beta-rhvae_$(ϵ)stepsize_$(lpad(epoch, 5, "0"))epoch.jld2",
