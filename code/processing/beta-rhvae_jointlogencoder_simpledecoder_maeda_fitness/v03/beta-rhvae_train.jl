...
# Define T values
T_vals = Float32.([0.25, 0.5, 0.65, 0.8, 1.0, 1.25, 1.5])
...
# List files
files = sort(Glob.glob("./output/model*.jld2"))

# Load models
models = [JLD2.load(file)["model"] for file in files]

# Extract model parameters

# Load parameters
model_states = [JLD2.load(file)["model_state"] for file in files]

# Input parameters to model
Flux.loadmodel!.(models, model_states)

# Update metric parameters
AutoEncode.RHVAEs.update_metric!.(models)
...
`n_epoch = $(n_epoch)`
## Training regimen
`beta-rhvae mini-batch training witm multiple dimensionalities`
...
Threads.@threads for i in eachindex(models)
    ...
    ldim = size(rhvae.vae.encoder.µ.weight, 1)

    # Update metric parameters
    AutoEncode.RHVAEs.update_metric!(rhvae)

    # List previous model parameters
    model_states = sort(Glob.glob("$(out_dir)/beta-rhvae_$(ldim)D_*epoch.jld2"))
    ...
    for epoch in epoch_init:n_epoch
        ...
        println("\n ($(ldim)D) Epoch: $(epoch) / $(n_epoch)\n - mse_train: $(mse_train)\n - mse_val: $(mse_val)\n")

        # Save checkpoint
        JLD2.jldsave(
            "$(out_dir)/beta-rhvae_$(ldim)D_$(lpad(epoch, 5, "0"))epoch.jld2",
