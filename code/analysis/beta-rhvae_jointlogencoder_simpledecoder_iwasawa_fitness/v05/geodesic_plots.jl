...
nng_files = Glob.glob("$(model_dir)/geodesic_state/*.jld2"[2:end], "/")
...
files = Glob.glob("$(model_dir)/gpu_model_state/beta*.jld2"[2:end], "/")
...
# Load model template
rhvae_template = JLD2.load("$(model_dir)/gpu_model.jld2")["model"]
...
for (i, temp) in enumerate(temps)
    ...
    model_files = Glob.glob(
        "$(model_dir)/gpu_model_state/*$(temp)*.jld2"[2:end], "/"
    )
    # Load parameters
...
latent_range = Float32.(range(-7, 7, length=n_points))
    ...
    for (j, data_strain) in enumerate(df_group)
		...
		aspect=AxisAspect(1),
            ...
            file = Glob.glob(
                ...
                "neuralgeodesic_$(temp)_$(env)env_" *
