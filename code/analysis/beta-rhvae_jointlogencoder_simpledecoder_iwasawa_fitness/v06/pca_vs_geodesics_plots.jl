...
files = Glob.glob("$(model_dir)/gpu_model_state/*$(T)temp*.jld2"[2:end], "/")

# Load model template
rhvae = JLD2.load("$(model_dir)/gpu_model.jld2")["model"]
...
model_files = sort(Glob.glob(
    "$(model_dir)/gpu_model_state/*$(T)*.jld2"[2:end], "/"
))
# Load parameters
...
latent_range_z1 = Float32.(range(-7.5, 5.5, length=n_points))
latent_range_z2 = Float32.(range(-5, 6, length=n_points))
...
for (k, env) in enumerate(envs)
    # Define file name
    fname = "$(out_dir)/geodesics_PCA_vs_RHVAE_$(env)env.pdf"
    ...
    for (j, data_strain) in enumerate(df_group)
        println("Processing $(env)env $(j)/$(length(df_group))")
        # Sort data by day
        DF.sort!(data_strain, :day)

        # Extract group metadata
        env = data_strain.env[1]
        strain = data_strain.strain[1]
        ...
        lines!(
            axes[1],
            [data_strain[1, :pc1], data_strain[end, :pc1]],
            [data_strain[1, :pc2], data_strain[end, :pc2]],
            linewidth=3,
