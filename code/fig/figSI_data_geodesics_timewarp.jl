...
# Define number of strains per figure
n_strains = 3

# Calculate number of figures needed
n_figures = ceil(Int, length(pairs) / n_strains)

# Loop through each figure
for fig_idx in 1:n_figures
    println("Creating figure $(fig_idx) of $(n_figures)")
    ...
    # Calculate start and end indices for pairs in this figure
    start_idx = (fig_idx - 1) * n_strains + 1
    end_idx = min(fig_idx * n_strains, length(pairs))

    # Number of strains in this figure
    n_strains_this_fig = end_idx - start_idx + 1

    # Add grid layouts as rows for each strain
    gl_strains = [gl[i, 1] = GridLayout() for i in 1:n_strains_this_fig]

    # Define number of rows and columns
    rows = 2
    cols = 4

    # Loop through each strain in this figure
    for i in 1:n_strains_this_fig
        # Extract pair
        p = pairs[start_idx+i-1]
    ...
    end # for i in 1:n_strains_this_fig

    # Save figure with index
    println("Saving figure $(fig_idx)...")
    save("$(fig_dir)/figSI_data_geodesics_timewarp_$(fig_idx).pdf", fig)
    save("$(fig_dir)/figSI_data_geodesics_timewarp_$(fig_idx).png", fig)
end # for fig_idx in 1:n_figures
