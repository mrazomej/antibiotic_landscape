...
# latent space dimensions when trained on antibiotic resistance data.
...
rhvae_dir = "$(git_root())/output/" *
            "beta-rhvae_jointlogencoder_simpledecoder_iwasawa_mcmc"

# Define list of versions
# NOTE:
# - v05: 2D latent space trained on 3D simulated data
# - v09: 3D latent space trained on 3D simulated data
versions = ["v05", "v09"]
...
Label(
    gl_banner[1, 1],
    "reconstruction error comparison for antibiotic resistance data",
    fontsize=12,
    padding=(0, 0, 0, 0),
    halign=:left,
...
# Set y limits
ylims!.([ax_train, ax_val], low=-0.025, high=0.5)
...
# Save figure
save("$(fig_dir)/figSI_data_dimension-error-comparison.png", fig)
save("$(fig_dir)/figSI_data_dimension-error-comparison.pdf", fig)
