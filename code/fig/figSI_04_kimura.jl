...
# Extract step size
step_size = DD.dims(fitnotype_profiles, :time)[2] - t_init
# Subsample time series
fitnotype_profiles = fitnotype_profiles[time=DD.At(t_init:n_sub*step_size:t_final)]
...
dist_dict = Dict(
    :phenotype => Distances.pairwise(
        ...
        StatsBase.transform(
            dt_dict[:phenotype],
            reshape(dd_join.phenotype[replicate=DD.At(1)].data, 2, :)
        )
    ),
    :rhvae => Distances.pairwise(
        ...
        StatsBase.transform(
            dt_dict[:rhvae],
            reshape(dd_join.rhvae[replicate=DD.At(1)].data, 2, :)
        )
    ),
    :vae => Distances.pairwise(
        ...
        StatsBase.transform(
            dt_dict[:vae],
            reshape(dd_join.vae[replicate=DD.At(1)].data, 2, :)
        )
    ),
    :pca => Distances.pairwise(
        ...
        StatsBase.transform(
            dt_dict[:pca],
            reshape(dd_join.pca[replicate=DD.At(1)].data, 2, :)
        )
    ),
)
...
# Initialize variables to store the minimum points and their metadata
min_left_point = nothing
min_right_point = nothing
min_left_value = Inf
min_right_value = Inf
    ...
    for time in DD.dims(pca_data, :time)
        for replicate in DD.dims(pca_data, :replicate)
            for evo in DD.dims(pca_data, :evo)
                ...
                pca_coords = R_dict[:pca] * StatsBase.transform(
                    dt_dict[:pca], pca_coords
                )

                # Check if the x-coordinate is less than zero and update
                # minimum left point
                if (pca_coords[1] < 0) && (pca_coords[2] < min_left_value)
                    global min_left_value = pca_coords[2]
                    global min_left_point = (lineage, time, replicate, evo)
                end

                # Check if the x-coordinate is greater than zero and update
                # minimum right point
                if (pca_coords[1] > 0) && (pca_coords[2] < min_right_value)
                    global min_right_value = pca_coords[2]
                    global min_right_point = (lineage, time, replicate, evo)
                end
            end
        end
    end
...
plot_point!(ax_pca, :pca, min_left_point, Antibiotic.viz.colors()[:dark_blue])
plot_point!(ax_pca, :pca, min_right_point, Antibiotic.viz.colors()[:dark_red])

plot_point!(ax_vae, :vae, min_left_point, Antibiotic.viz.colors()[:dark_blue])
plot_point!(ax_vae, :vae, min_right_point, Antibiotic.viz.colors()[:dark_red])

plot_point!(ax_rhvae, :rhvae, min_left_point, Antibiotic.viz.colors()[:dark_blue])
plot_point!(ax_rhvae, :rhvae, min_right_point, Antibiotic.viz.colors()[:dark_red])

plot_point!(ax_phenotype, :phenotype, min_left_point, Antibiotic.viz.colors()[:dark_blue])
plot_point!(ax_phenotype, :phenotype, min_right_point, Antibiotic.viz.colors()[:dark_red])
...
# Save figure
save("$(fig_dir)/figSI_04_kimura.pdf", fig)
save("$(fig_dir)/figSI_04_kimura.png", fig)
