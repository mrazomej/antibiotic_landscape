...
for stage in 1:5
    println("Plotting Fig06 stage $(stage)...")
    # Initialize figure
    ...
    Box(
        ...
        color="#E6E6EF",
        strokecolor="#E6E6EF",
        alignmode=Mixed(; left=-20, right=-50), # Moves box to the left and right
    )
        ...
        if i == length(pairs)
            if stage ≥ 2
                ...
                Legend(
                    ...
                    tellwidth=false,
                    # halign=:left,
                    alignmode=Mixed(; left=50, right=10, top=0, bottom=0),
                )
            end # if stage ≥ 2
            # Add colorbar
            Colorbar(
                ...
                hm,
                label="√log[det(G)]",
                labelcolor=stage ≥ 3 ? :black : :white,
                ticklabelsvisible=stage ≥ 3 ? true : false,
                spinewidth=stage ≥ 3 ? 1.0 : 0.0,
                tickcolor=stage ≥ 3 ? :black : :white,
            )
        end # if i == length(pairs)
    end # for p in pairs
    ...
    # Save figure
    save("$(fig_dir)/fig06_$(lpad(stage, 2, '0'))stage.png", fig)
    save("$(fig_dir)/fig06_$(lpad(stage, 2, '0'))stage.pdf", fig)
