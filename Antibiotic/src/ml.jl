...
- `latent_activation::Vector`: Array containing the activation functions coming
  in and out of the latent space layer.
...
function vae_init(
    n_input::Int,
    n_latent::Int,
    latent_activation::Vector,
    encoder::Vector{Int},
    encoder_activation::Vector,
    decoder::Vector{Int},
    decoder_activation::Vector,
)
    ...
    for i = 1:length(encoder)
        ...
        if i == 1
            ...
            if typeof(encoder_activation[i]) <: Nothing
                # Set first layer from input to encoder
                Encoder[i] = Flux.Dense(n_input => encoder[i])
            else
            ...
            # Check if there's an activation function
            if typeof(encoder_activation[i]) <: Nothing
                # Set middle layers from input to encoder
                Encoder[i] = Flux.Dense(encoder[i-1] => encoder[i])
            else
    ...
    # Check if latent space has activation function
    if typeof(latent_activation[1]) <: Nothing
        # Define layer that maps from encoder to latent space
        Latent_µ = Flux.Dense(encoder[end] => n_latent)
        Latent_logσ = Flux.Dense(encoder[end] => n_latent)
    else
        # Define layer that maps from encoder to latent space with activation
        Latent_µ = Flux.Dense(
            encoder[end] => n_latent, latent_activation[1]
        )
    ...
    # Check if latent space to decoder layer has activation
    if typeof(latent_activation[2]) <: Nothing
        # Add first layer from latent space to decoder
        Decoder[1] = Flux.Dense(n_latent => decoder[1])
    else
        # Add first layer from latent space to decoder
        Decoder[1] = Flux.Dense(n_latent => decoder[1], latent_activation[2])
    end # if

    # Check if last layer has activation
    if typeof(decoder_activation[end]) <: Nothing
        # Add last layer from decoder to output
        Decoder[end] = Flux.Dense(decoder[end] => n_input)
    else
        # Add last layer from decoder to output
        Decoder[end] = Flux.Dense(
            decoder[end] => n_input, decoder_activation[end]
        )
    end # if
    ...
    if length(decoder) > 1
        ...
        for i = 2:length(decoder)
            # Check if layer has activation
            if typeof(decoder_activation[i-1]) <: Nothing
                # Set middle layers of decoder
                Decoder[i] = Flux.Dense(decoder[i-1] => decoder[i])
            else
                # Set middle layers of decoder
                Decoder[i] = Flux.Dense(
                    decoder[i-1] => decoder[i], decoder_activation[i-1]
                )
            end # if
        end # for
    end # if
...
function vae_reconstruct(
    ...
    #    standard deviation
    z = µ .+ (Random.rand(Distributions.Normal{Float32}(0.0f0, 1.0f0), length(µ)) .*
              exp.(logσ))
