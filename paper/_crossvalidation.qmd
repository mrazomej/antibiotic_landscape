...
To rigorously evaluate whether nonlinear latent space coordinates capture more
...
(PCA/SVD) and nonlinear (VAE and RHVAE) dimensionality reduction techniques.
...
For nonlinear models (VAE and RHVAE), we adapt the cross-validation approach to
...
To fairly compare the predictive power of linear and nonlinear dimensionality
...
the main text, demonstrates that for all antibiotics, the 2D nonlinear latent
space coordinates provide more accurate predictions than any number of linear
dimensions.

The key finding is that the nonlinear latent spaces capture the underlying
...
nonlinear structure that is better represented by VAE and especially RHVAE
models.

### Cross-Validation comparison of 2D and 3D latent spaces

In Figure 7 in the main text, we tested the predictive power of 2D nonlinear
...
![**Comparison of 2D and 3D nonlinear latent space models for predicting
...
Horizontal lines represent the accuracy of nonlinear models: 2D-VAE (dark blue,
