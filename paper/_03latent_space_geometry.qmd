...
we have developed so far, the hope is that the low-dimensional latent spaces
capture features of the structure of the underlying phenotypic space.
...
of how such non-linear transformations affect the geometry of the learned space
[@arvanitidis2018]. This is analogous to having a 2D map of the surface of the
surface of the earth (the latent space), where, to know how distances between
two points on the map correspond to distances on the surface of the planet (the
data space), we need a position-dependent "scale bar"---more formally, a metric
tensor---that allows us to convert distances between representations. This
metric tensor is not encoded in the original VAE framework, rendering distances
between points in the latent space meaningless as they are not directly related
to the distances in the data space.
...
profiles. It is important to highlight that at no point the RHVAE obtains any
knowledge about the identity of the input data. In other words, the RHVAE has no
information about the lineage or time point of the fitness profile that it is
processing. The network only uses the unidentified fitness profiles to learn the
