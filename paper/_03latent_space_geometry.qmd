...
techniques implicitly assume that the positions of points in the data space is
...
The goal of dimensionality reduction is to find such embedded manifold that
...
the visualization of the underlying structure data structure. Given the
conceptual picture we have developed so far, the hope is that the
low-dimensional latent spaces captures features of the structure of the
...
surface of the earth (the latent space) where, to know how distances between two
points on the map correspond to distances on the surface of the earth (the data
space), we need a position-dependent "scale bar"---more formally, a metric
...
between points in the latent space meaningless as they are not directly
related to the distances in the data space.
