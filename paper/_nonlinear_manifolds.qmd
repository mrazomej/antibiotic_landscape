...
## Advantages of Nonlinear Manifold Learning for Fitness Landscapes {#sec-nonlinear}

In this section, we provide a theoretical justification for why nonlinear
...
### Mathematical Advantages of Nonlinear Manifolds
...
causal manifold has intrinsic dimension $d'$ but is nonlinear, capturing its
...
$\underline{\underline{G}}(\underline{z})$. On a nonlinear manifold, this
...
To learn nonlinear manifolds directly, we employ variational autoencoders
...
### Comparative Advantages of Nonlinear Methods

Nonlinear methods offer several theoretical advantages over linear approaches:

1. **Dimensionality Efficiency**: Nonlinear methods can potentially represent
the same information with fewer dimensions. If the true causal manifold has
intrinsic dimension $d'$ but is nonlinear, linear methods may require $d > d'$
dimensions to achieve comparable accuracy.

2. **Geometric Fidelity**: By learning the metric tensor directly, nonlinear
...
is nonlinear, nonlinear methods should provide better predictive accuracy,
...
explicitly rather than through additional dimensions, nonlinear methods can
provide a more compact and interpretable representation of the fitness
landscape.

### Empirical Evidence and Caveats

While the theoretical advantages of nonlinear methods are clear, empirical
validation is essential. We observe this advantage in practice through several
metrics:

1. **Reconstruction Error**: Nonlinear methods achieve lower reconstruction
...
matrix suggests inherent nonlinearity in the fitness landscape.

3. **Generalization Performance**: Nonlinear methods show better performance
when predicting fitness for held-out genotypes.

However, it's important to note that nonlinear methods come with their own
challenges:

1. **Overfitting Risk**: Nonlinear methods may introduce spurious complexity
when the true structure is simple.

2. **Interpretation Challenges**: The biological meaning of nonlinear latent
dimensions can be less straightforward than those from linear methods.

3. **Training Complexity**: Nonlinear methods typically require more data and
computational resources for effective training.

4. **Validation Requirements**: Demonstrating that a learned nonlinear
...
structure to ensure biological relevance of our nonlinear representations.