...
$g$ is mapped to a phenotype coordinate $\underline{p}$ via a function
$\phi$---depicted as two potential molecular phenotypes such as an eflux pump
rate and the sensitivity of a membrane receptor. The fitness of each genotype is
determined by the value of these phenotypes and the environment $E$ via a
function $F$---depicted as the doubling time of a bacterial population. To
mathematize this conceptual model, we assume that phenotypes can be described by
a vector of $P$ real-valued variables, $\underline{p} \in \mathbb{R}^P$. Thus,
phenotypic space is a $P$-dimensional Euclidean space, and any genotype $g$ has
a unique mapping to a phenotypic coordinate. However, this mapping is not
necessarily bijective, i.e., multiple genotypes may map to the same phenotype.
To represent the likelihood of sampling some genotype $g$ that maps to some
coordinate $\underline{p}$ in phenotype space, we introduce a
genotype-to-phenotype density map $GP: \mathbb{R}^P \to \mathbb{R}$. This
function captures the empirical observation that the genotype-to-phenotype map
is highly redundant, with many genotypes mapping to equivalent phenotypes and
some phenotypes being extremely rare [@louis2016]. @fig-01(B, left) shows a
schematic of one possible $GP$ map overlaid on a 2D phenotype space. For
illustration purposes, we set the $GP$ map to contain four negative Gaussian
peaks, which represent four regions of the phenotype space that are unlikely to
be sampled (see Supplementary Material for a more detailed discussion). The
location and size of these peaks are assumed to be fixed by the
genotype-to-phenotype map, irrespective of the environment. We also introduce a
fitness function $F: \mathbb{R}^P \to \mathbb{R}$ that maps phenotypes to
fitness values. However, the topology of the fitness landscape is determined by
the environment $E$. @fig-01(B, right) shows a schematic of the simplest fitness
landscape with a single symmetric Gaussian peak.
...
environment varies. We model this by changing the topology of the fitness map in
terms of the number, position, height, and width of the Gaussian peaks.
...
hereafter referred to as a fitness profile. The challenge, then, is to extract
information about these latent phenotypic coordinates from such high-dimensional
fitness profiles.
...
genotype-phenotype density contours. (D) Fitness profiles. The fitness values of
any genotype measured across multiple environments as an experimental
