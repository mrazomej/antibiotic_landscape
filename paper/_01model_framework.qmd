...
to serve as a guidepost for our interpretation of the data. 
...
$\phi$---depicted as two potential molecular phenotypes, such as an efflux pump
...
formalize mathematically this conceptual model, we assume that phenotypes can be
described by a vector of $P$ real-valued variables, $\underline{p} \in
\mathbb{R}^P$. Thus, phenotypic space is a $P$-dimensional Euclidean space, and
any genotype $g$ has a unique mapping to a phenotypic coordinate. However, this
mapping is not necessarily bijective, i.e., multiple genotypes may map to the
same phenotype. To represent the likelihood of sampling some genotype $g$ that
maps to some coordinate $\underline{p}$ in phenotype space, we introduce a
...
peaks, representing four regions of the phenotype space that are unlikely to be
sampled (see Supplementary Material for a more detailed discussion). The
