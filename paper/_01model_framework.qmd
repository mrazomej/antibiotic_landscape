...
fitness values. However, the topography of the fitness landscape is determined
by the environment $E$. @fig-01(B, right) shows a schematic of the simplest
fitness landscape with a single symmetric Gaussian peak.
...
hereafter referred to as a fitness profile. We ask whether it is possible to
extract information about the latent phenotypic coordinates based only on such
high-dimensional fitness profiles. Conceptually, this is equivalent to asking
whether it is possible to reverse the mapping from phenotypes to fitness values
and predict the phenotypic coordinates of a genotype based only on its fitness
values on multiple environments.
...
across multiple environments as an experimental
