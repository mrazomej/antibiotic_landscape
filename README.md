...
- **Paper**: [Learning the Shape of Evolutionary Landscapes: Geometric Deep
  Learning Reveals Hidden Structure in Phenotype-to-Fitness
  Maps](https://mrazomej.github.io/antibiotic_landscape/paper.html)
- **Preprint**:
  [bioRxiv](https://www.biorxiv.org/content/10.1101/2025.05.07.652616v1)
...
- **`geometry.jl`**: Procrustes analysis, Fréchet distance calculations, and geometric transformations
- **`metropolis.jl`**: Metropolis-Hastings evolutionary dynamics simulation with Gaussian fitness landscapes
...
- **`beta-rhvae_jointlogencoder_simpledecoder_iwasawa_mcmc/`**: RHVAE training and analysis on Iwasawa dataset
...
data = load_fitness_data("path/to/data.csv")

# Train RHVAE model
model = train_rhvae(data, 
    latent_dim=2,
    hidden_dim=128,
    temperature=0.5,
    regularization=1e-2
...
# Analyze results
latent_repr = encode(model, data)
geodesics = compute_geodesics(model, latent_repr)
```
...
# Run Metropolis-Hastings evolution
trajectory = evolve_population(
    initial_population,
    peaks,
    n_steps=1000,
    mutation_rate=0.01
